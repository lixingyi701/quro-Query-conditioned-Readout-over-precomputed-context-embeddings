...
### 10.5.6 ⭐ go/no-go 首轮结果（3000 步，1000 条评测，2026-09-15）

四个 arm 各训 3000 步、单卡并行。**全集上的预注册判据判为 NO-GO，但该判据的指标本身无效**，理由见下。

**文档错配对照（决定性）**：保持 query 与标准答案不变，把文档换成隔壁样本的。

```
trivia|D0|B=8               EM=67.90%   ← 正确文档
trivia/mismatch-doc|D0|B=8  EM=64.10%   ← 错误文档
```

**整条检索+压缩链路只值 3.8 个 EM 点**；64.1% 是 Mistral-7B 靠参数化知识答出来的。TriviaQA 上 94% 的分数与被测对象无关，而四个 arm 的差距全部落在这 3.8 分带内。**没有这个对照，任何 RAG 压缩的消融数字都不可解释**——这一条对 related work 里那些不报无证据下界的工作同样适用。

**修正后的指标**：用「拿错文档也能答对」把题筛掉（500 条中占 58%），只在剩下 210 条证据依赖题上重算（`scripts/evidence_subset.py`，纯重分析，零额外算力）：

| arm | 送进生成器 token | 全集 EM | **证据子集 EM** |
|---|---:|---:|---:|
| **P** PISCO 原生直喂 | ~48 | 64.80% | **35.71%** |
| **S** 相似度 top-B（零可训参数） | 8 | 63.40% | **28.10%** |
| **C** xattn readout（ours） | 8 | 62.20% | **18.10%** |
| **A** agnostic readout | 8 | 60.00% | **17.14%** |

差距从 4 分带放大到 **19 分带**，排序才有了意义。SeleCom dev 上同向（P 11.11 / S 10.22 / C 6.44 / A 5.56）。

**核心诊断：训练过的 readout 丢掉了 S 保住的一半证据信号。**

结构性差异在于 **S 把 latent 逐行原样取出，C/A 交的是注意力加权混合**。若干 Mistral 隐状态的凸组合未必仍是合法的 Mistral 隐状态，对 decoder 是 off-manifold 的；而 PISCO 的 decoder 恰恰是在真实压缩 embedding 上训出来的。§2.3 的残差设计还偏偏**从最大混合出发**（初始注意力熵 = 均匀的 99.85%），等于把模型放在了最不利的起点。

`C − A = +0.96`：query 条件化的收益方向正确但仍远低于 3 分判据。

**下一步应当做的实验（按优先级）**：

1. **稀疏/硬选择 readout**。S 本质上是我们架构在「硬选择 + 余弦打分」下的特例。把 readout 改成学习到的硬选择（低温 softmax / top-k straight-through / Gumbel），预期应当超过 S——这是最直接的可证伪假设，也最便宜。
2. **重做评测集**。TriviaQA 的闭卷可答率太高。改用 PopQA（长尾实体，设计文档 §2.3 已列出正是为此），或固定采用「闭卷答错子集」作为主指标。
3. **stage 2（query 明文 dropout）**。D0 下 decoder 拿着明文 query，会系统性压低 C − A；D1 才是 query 条件效应的上界估计。
4. 判据阈值需要**在有效指标上重新预注册**，不能沿用在被污染指标上定的数。

### 10.5.8 ⭐ 完整主表：加入 AG / RG 上下界与 prefill 成本轴

§10.5.6 缺了两行关键对照。补齐后（substring 指标，同一 PISCO-adapted decoder，统一取前 500 条）：

**TriviaQA**

| 配置 | prefill token | substring | 证据增益 | 保留率 | 增益/额外 token |
|---|---:|---:|---:|---:|---:|
| **AG** 闭卷无证据 | 56 | 62.40% | — | — | — |
| A agnostic readout | 70 | 66.00% | +3.6 | 30% | 0.257 |
| **C QuRO xattn** | 70 | 66.80% | +4.4 | 36% | **0.314** |
| S similarity top-B | 70 | 69.00% | +6.6 | 54% | 0.471 |
| P PISCO 直喂 | 115 | 72.40% | +10.0 | 82% | 0.169 |
| **RG** 未压缩原文 | 771 | 74.60% | +12.2 | 100% | 0.017 |

**SeleCom dev**

| 配置 | prefill | substring | 证据增益 | 保留率 | 增益/额外 token |
|---|---:|---:|---:|---:|---:|
| AG 闭卷 | 52 | 14.00% | — | — | — |
| A | 66 | 17.40% | +3.4 | 26% | 0.243 |
| **C QuRO** | 66 | 18.40% | +4.4 | 34% | **0.314** |
| S | 66 | 20.80% | +6.8 | 52% | 0.486 |
| P PISCO | 111 | 22.40% | +8.4 | 65% | 0.142 |
| **RG** | 794 | 27.00% | +13.0 | 100% | 0.018 |

**读法（这才是本方案该有的立论方式）**：按「每多花一个 prefill token 换来多少证据增益」，**QuRO 是 PISCO 的 2.2 倍、未压缩 RAG 的 17 倍**。12 倍短的 prefill 换来 34~36% 的证据保留率。绝对精度落后是预期之内的，效率轴才是主场。

**但两条约束不变**：

1. **S 在两个轴上都压着 C**（保留率 52% vs 34%，效率 0.49 vs 0.31）。「短 prefill + 缓存读出」这个**架构族**确实高效，但我们这个具体的 readout 不是族里最好的成员。换效率视角不改变这个结论。
2. **TriviaQA 证据天花板只有 12.2 分**（闭卷 62.4 → 未压缩 74.6），判别力弱；dev 上 RG 几乎翻倍（14.0 → 27.0），主实验应以后者一类的数据为主。

**两个必须记住的评测陷阱（都是本轮自己踩的）**：

- **跨格式基线只能看 substring/F1**。plain Mistral 与 PISCO-adapted decoder 的闭卷能力实测相同（63.20% vs 62.40% substring），但 EM 相差 280 倍（0.20% vs 56.40%）——差异**纯粹是输出啰嗦程度**，PISCO 把 decoder 微调成了简短作答。用 EM 比较未适配基线等于在惩罚它的输出风格。
- **RG 的 prompt 截断会静默切掉问题**。证据在前、问题在后，整体 `truncation=True, max_length=2048` 会先砍掉问题和生成提示。第一版 trivia RG 因此跑出 0.80%（**低于闭卷**）。修法是逐篇裁剪文档（裁到压缩器同样看到的 128 token，这同时也让 RG 成为"同样证据、不压缩"的真上界），并在整体仍超长时直接报错而非静默截断。
...
## 10.6 v0.2 设计决策：把 query 搬进 Mistral 空间，让打分从余弦出发

§10.5.6 的诊断定位到瓶颈是**选错文档**（gold 注意力 39.3%，余弦 67.6%，随机 25.4%），根因是两个 arm 拿到的表示不对等。本节记录据此定下的改法与**推导过程中被推翻的两个说法**，以免日后重犯。

### 10.6.1 被推翻的说法一：「注意力加权混合对 decoder 是 off-manifold」

曾用「凸组合不在 Mistral 隐状态流形上」解释 C 输给 S。**该解释不成立**，三条反证：

1. COCOM-light 的 encoder 是 BERT（768 维，另一个模型），靠 projector 映射进 Mistral 空间就能工作——跨空间映射本来就是可学的；
2. 我们的 generator LoRA（41.94M）**本身在训练**，decoder 有充分机会适应 readout 的输出分布；
3. 实测（`scripts/check_readout_stats.py`）训练后 `‖E‖/平均‖Z‖ = 0.886`、与最近缓存 latent 余弦 **0.895**、有效支撑从 42.2 降到 **3.64**——输出的尺度、方向、稀疏度都正常。

而且有个更强的逻辑约束：**C 的表达能力严格包含 S**（注意力取 one-hot 即复现 S）。表达能力更强却更差，只能是**优化与归纳偏置**问题，不是表示问题。用「流形」这类表示层面的词解释是错的。

### 10.6.2 被推翻的说法二：「残差初始化让第 0 步 ≈ PISCO」

原设计 `E = base_scale · AttnPool(α, Z) + Δ`（Δ 零初始化），当时声称第 0 步等于「注意力池化的 PISCO」。**字面为真，含义为假**：PISCO 是把 40 个 latent **全部**交给 decoder，而第 0 步给出的是 40 个 latent 的**平均值复制 B 份**。两者完全不同——我以为从可用解出发，实际从一团 mush 出发。

**为什么随机初始化必然导致平均**（这点反直觉，记下来）：语义在 V 里，但注意力权重由另外的随机矩阵算。`softmax` 不是"随机挑一个"，而是"**分数都差不多时平均分配**"。随机初始化 + LayerNorm 输入 + 除以 √d，40 个 logit 全挤在 0 附近 → 每个约 1/40。实测初始注意力熵 = 均匀的 **99.85%**。

叠加第二层原因：即使 logit 够大，q 来自 Qwen3-Embedding 空间、z 来自 Mistral 空间，**两个不相干坐标系各过一个独立随机投影**，内积与语义无关（JL 引理只在同一空间内保距）。

**真正对应「从可用解出发」的是 S，而初始化恰恰没落在那里。** 残差技巧只做在了输出端，漏了打分端。

### 10.6.3 改法

删除 Qwen3-Embedding-0.6B，query 的两种表示都从 Mistral 取（那次前向本来就要付，`pool_query_in_generator_space` 之前只是把逐 token 隐状态平均掉了——平均是 arm S 的余弦规则所要求的，不是表示上的选择）。

注意力打分改为：

```
logit[b,j] = ⟨ W_q·(q̄ + s_b + xattn_b),  W_k·z_j ⟩ · τ
                    ↑      ↑        ↑
              共享余弦   槽位分化   细节修正（零初始化）
```
...
| 成分 | 来源 | 初始化 | 作用 |
|---|---|---|---|
| `q̄` | Mistral 池化 query，过**与 latent 相同的 `in_proj`** | `W_q/W_k` 近恒等，τ≈20 | 初始打分即余弦，等价 S 的打分规则 |
| `s_b` | 可学 slot embedding | 小初始化 | 提供 **b 维度**，让 8 个槽分工 |
| `xattn_b` | Mistral 逐 token 隐状态 `(T,4096)` 走 cross-attn | **零初始化** | 保留问题多侧面的精细分工 |

三个要点：

1. **共享同一个 `in_proj` 才是关键**，不是「都来自 Mistral」就够。同一个线性映射作用于两侧，内积才近似保持原空间几何。
2. **温度 τ 必须初始化为大值**（余弦值域仅 [-1,1]，不放大则 softmax 仍然均匀）。
3. **池化方式改为可配**（`last` / `mean` / `weighted`），默认 `last`：Mistral 是因果模型，末 token 隐状态注意过整个问题，是 decoder-only 模型的标准句表示（E5-mistral、RepLLaMA 同法）；latent 本身也来自 mem token 位的末层隐状态，性质一致。`mean` 留作消融——arm S 现有的 67.6% 可能正被 mean-pool 拖累。

### 10.6.4 必须说清的两个限制

- **第 0 步不等于 S**。`τ·cos(q̄, z_j)` 不含 `b`，对 8 个槽是同一行，softmax 后 8 个槽输出同一向量。而 S 的多样性来自**硬性 top-B 排序赋值**（第 b 槽取第 b 名），软注意力做不到。准确表述是「第 0 步 = S 的软版本、去掉槽位多样性」：所有槽都集中到 query 相关的 latent 上，多样性需靠 `s_b` 与 slot self-attention 学出。预期 gold 注意力从 39.3% 升向 67.6%，但槽间冗余需单独监控（用 `check_readout_stats.py` 的 `effective_support` 与槽间余弦）。
- **在线成本增加一次 query 前向**（约 15 token）。arm S 本来就在付，对比公平，但效率表必须计入：在线总量从 70 token 变 85 token，相对 RG 的 794 仍便宜 9 倍以上。

### 10.6.5 本轮的判据

改完若 **C > S**，方法成立，继续主实验。
改完若 **C ≈ S**，则证实 §10.5.6 的理论顾虑：m=8 时在线选择本质就是重排序，没有额外空间；此时必须上 COCOM rate 4（m=32，K·m=160 选 8）才有发挥余地。**两种结果都有信息量**——后者把关键变量精确定位到 ξ_off。

---

## 10.7 v0.2 结果与 ξ_off 实验设计（2026-09-15 晚）

### 10.7.1 v0.2 readout 的结果：追平 S，未能超越

TriviaQA 全量 2000 条、B=8、D0、同一 decoder：

| arm | prefill | substring | EM |
|---|---:|---:|---:|
| P PISCO 直喂 | 115 | 77.20% | 70.75% |
| S 相似度 top-B | 70 | 76.10% | **71.70%** |
| C v0.2 先验·rank | 70 | 76.00% | 70.90% |
| C v0.2 先验·shared | 70 | 75.15% | 70.25% |

配对检验（McNemar 精确检验，n=2000）：

| 对比 | EM | substring | 结论 |
|---|---|---|---|
| C·rank vs S | −16 净, p=0.32 | −2 净, **p=0.94** | **完全打平** |
| S vs P | +19 净, p=0.24 | −22 净, p=0.13 | 三指标方向不一致，**无差异** |
| C·rank vs P | +3 净, p=0.91 | −24 净, p=0.13 | 无显著差异 |

**三条必须记住的教训**：

1. **n=500 的点估计会骗人**。同一对比在 n=500 时是 C 领先 1.6 分（看起来赢了），n=2000 时是 99 胜 101 负。小样本上先抛点估计再补检验，顺序是错的——检验要先做。
2. **挑指标会骗人**。"S 的 EM 超过 PISCO"只在三个指标中的一个上成立（p=0.24），substring 和 F1 都反向。**报结论必须三个指标一起报。**
3. **v0.2 相对 v0.1 是真实提升**（n=500 时 p=0.033，从 66.80% 到 76.00%/全量），余弦先验在初始化点值 +18 分。**v0.1 的初始化比不给证据还差 15.4 分**——那是个负分起点。

**这命中了 §10.6.5 预注册的第二种情形**：C ≈ S ⇒ m=8 时在线选择本质是重排序 ⇒ 必须验证 m 增大后是否出现收益。

**附带确认**：槽位冗余不要钱。`shared` 让 8 个输出槽的两两余弦 = 1.000（完全相同），与 `rank` 的差距仍不显著（p=0.17）。xRAG 用单个 soft token 作答是同一个观察。先前"浪费 B−1 预算"的担心不成立。

### 10.7.2 ξ_off 实验：两条互相独立的 m=32 路线

**PISCO 的 m 改不了**：`n_mem_tokens = doc_max_length // compr_rate` 被 8 个**训练过的** `<MEM0..7>` embedding 钉死，`add_memory_tokens_to_inputs` 还有断言。改 `compr_rate` 只会触发断言——`<MEM8>` 不存在也从未训练。喂更长的文档确实能改 ξ_off，**但方向反了**：m 仍是 8，`K·m` 不变，选择问题难度分毫未动。

因此用两条路线各自得到 m=32，**并要求结论对两者同时成立**：

| 路线 | 实现 | 混淆变量 |
|---|---|---|
| **COCOM-v1-4** | rate=4，同一 Mistral 基座 | 换了 checkpoint |
| **PISCO 切块** | 128 token 切 4 段 × 32，各压出 8 个 latent | 每块独立压缩，**丢失跨块上下文** |

两者 ξ_off 都是 4.0、喂进去的 token 数都是 128。**混淆变量完全不同，同向结论才可信。**

切块路线实测通过：形状 (·,32,4096)、std 1.776（PISCO 原生 1.892，尺度相当，残差与余弦先验的假设仍成立）、文档间距离 743、**同文档 4 个块之间距离 350**（确实产生了文档内结构）。
...
- `build_latent_cache.py --offset` + `pack_latent_cache.py --merge`：一份缓存可切片给多卡并行构建再合并。m=32 的缓存 69.2GB，单卡约 86 分钟，双卡减半。
- **`hf_transfer` 把 HF 下载从 10 MB/min 提到 1913 MB/min**（190 倍）。14.5GB 的 checkpoint 从 22 小时变成 8 分钟。以后下大模型先装它。

---

## 10.8 ξ_off 实验结论：两条独立路线一致否定（2026-09-16）

### 10.8.1 结果

TriviaQA 全量 2000 条，B=8，D0，两条互相独立的 m=32 路线。**绝对数值不可跨栈比**（encoder/decoder 整套不同，证据增益 P 臂 +9.80 vs +6.95 即为证），可比的是**结论**。
...
| A agnostic（不看 query） | 8 | 73.90% | 69.00% | 0.745 | 67.55% | +6.35 |
| C ours | 8 | 73.70% | 68.55% | 0.740 | 67.30% | +6.40 |
| S 余弦 top-B（0 参数） | 8 | 73.40% | 69.15% | 0.742 | 67.95% | +5.45 |
...
| P vs C | **+104 sub (p<0.0001)** | **+71 sub (p<0.0001)** |

COCOM 栈里 **A 在 EM 上显著优于 C**；substring 无差异，两指标不一致故报「无可靠差异」，但方向为负。

### 10.8.2 可证伪预测被推翻

§10.5.4 预注册的预测是「收益随 K·m/B 单调增长」：
...
按 §10.6.5 的预注册判据，**C ≈ S 成立**。而且证据比 m=8 时更强，因为多了一条：**C ≈ A**——连「读出时看不看问题」都不重要。

架构不是约束条件。三个差异极大的 readout（学出来的+看 query、学出来的+不看 query、零参数余弦）落在 0.5 分以内，而「压不压缩」值 5~7 分（p<0.0001）。**损失发生在「175 → 8」这个动作本身，与「怎么挑这 8 个」无关。**

推论：**在 B=8 这个预算下，在线 query 条件选择退化为重排序，而重排序有零参数解。** 继续优化选择规则是在优化一个实测为平坦的维度。

### 10.8.4 下一步（按诊断力排序，非按工作量）

1. **扫 B**，而非改架构。目前只有 B=8 一个点，真正的问题是「C 随 B 增大能否逼近 P」。budget dropout 已实现，`budget_buckets=[4,8,16,32]` 一次训练给四个点，成本与现在一臂相同。若 C(B=32) ≈ P，方法成立、B=8 只是压得太狠；若仍落后，readout 才是结构性有损，那时改架构才有的放矢。
2. **换测试床**。TriviaQA 的证据天花板只有 12.2 分（闭卷 62.4 → 未压缩 74.6），我们在里面比高低本就不合适。`/home/lxy/selecom/data/nq/dev/` 有 7 分片 ≈ 7800 条 **Natural Questions**，是原始 Google 格式（整篇维基 HTML + `long_answer_candidates` + `annotations`），需要转换：从 `annotations` 取 short answer，`long_answer` 段落作 gold passage，**同一篇维基的其他段落作干扰项**。两个好处——NQ 闭卷正确率远低于 TriviaQA，证据空间大得多；干扰项是同主题的硬负例，比现在的随机干扰更能考验选择。规模约 47k 段落，m=32 缓存仅 12GB。
3. **简化 readout**（仅为精简，不指望涨点）。`q → B 个线性投影 → 一次交叉注意力`，砍掉可学 slot embedding、query 的 xattn block、槽间自注意力，参数 33.7M → 约 8M。依据 C≈A≈S，预期打平；打平即为更好的论文。
4. 若 B 曲线显示 C 无法逼近 P，则按设计文档 §9 把论文重心移到摊薄叙事。
