...
2. **加入 conflict instruction 后，模型是服从指令还是被文档记忆拉回？**
   被拉回，**但只在去掉 PISCO 脚手架之后才测得出来**。
   用 SeleCom 原文的 prompt 写法（无 system prompt、无 `Question:` 标记）：同一 decoder、
   同一条指令、同一个 nonce，只换文档表示——raw text 65% 遵循，PISCO memory **22.5%**，
   差 **−0.425 [−0.575, −0.275]**；不给任何背景时 100%。
   SeleCom Figure 2(a) 的**行为现象在 PISCO 上复现了**。
   **但在 `P` baseline 的实际服务配置下（PISCO system prompt + `Question:` + K=10 +
   真实问题），memory − raw = +0.015 [−0.045, +0.075]，CI 含 0。** PISCO 的 system prompt
   本身就命令模型"从提供的文档里抽取信息"，已经把所有条件压到 ≤5%，压缩的边际贡献被淹没
   （§2.3、§4.2）。**instruction suppression 不是 QuRO 当前性能瓶颈的解释。**
...
4. **主因更像 token 数量、QK/logit 尺度、向量范数、位置效应，还是根本不是 attention capture？**
   **根本不是 attention capture。** SeleCom 归因链的前提为真、推论为假：
   ...
   放大不存在。
   最直接的一条：把 latent 缩放 α 从 0.02 扫到 1.00（50 倍），遵循率走完 0.625 → 0.175
   的全程，而 density 比值只从 0.239 动到 0.278、mass 只动 15%——
   **行为摆 3.6 倍，注意力统计基本是常数。**
...
5. **因此下一模块具体要改变哪个量，并保留什么能力？**
   **现在还不能提出模块**（§6.1）：指示 §15 的门槛第 3、4 条未满足——没有任何内部机制
   跨样本稳定地预测失败，而唯一能改变行为的干预（随机化 latent 方向）把注意力优势推过 1
   之后行为反而**变好**，与预期相反。

   已确立的是一条**否定性方向约束**和一条**正向线索**：
   - 否定：任何以"降低 / 校准 memory 注意力通道"为动作的模块，都在改一个被证明与失败
     无关的量（§3、§5.1）；缩放也不行，压制与文档能力的拐点不同且方向不利（§5.1）。
   - 正向：压制由 **latent 分布的方向**承载，且与文档信息**不可分**——随机化方向能解除
     压制（0.225 → 0.625），代价是文档彻底丢失（ROUGE-L 0.616 → 0.118）；
     而 adapter 消融显示 PISCO 的 decoder LoRA 给 raw/空槽位加了 +0.65/+0.55 的指令遵循
     能力，给 memory/mismatch 加的是 0 到负（§5.2）。**下一步该攻的是"方向能否与文档
     信息分离"和"adapter 的训练目标"，不是注意力。**
...
| `levelA-intervene-pisco` / `-selecom` | A-intervene | 两种 | 40 doc × 18 条件 | Phase 3 最小可逆干预 |
| `levelA-intervene-lowalpha` | A-intervene | SeleCom | 40 doc × 20 条件 | α ∈ {0.02…0.15} 低段扫描 |
| `levelA-noadapter-selecom` | A | SeleCom | 40 doc × 9 条件 | 关掉 `decoder_adapter` 的消融 |
...
### 5.1 α 低段扫描：行为摆 3.6 倍，注意力纹丝不动

§5 的 α ∈ {0.25, 0.5, 0.75} 是平的，而 α = 0（`zero`）完全恢复，所以拐点在 0.25 以下。
补扫 α ∈ {0.02, 0.05, 0.10, 0.15}（n=40，SeleCom 脚手架）：

| α | `leading` | 重建 ROUGE-L | 抄文档最长片段 | mass 比值 | **density 比值** | qk gap |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0.00（`zero`） | **0.625** | — | — | 0.353 | 0.802 | +0.103 |
| 0.02 | 0.400 | 0.255 | 4.6 | 0.106 | **0.239** | −1.110 |
| 0.05 | 0.300 | 0.254 | 5.0 | 0.116 | **0.263** | −1.046 |
| 0.10 | **0.175** | 0.334 | 7.7 | 0.117 | **0.265** | −1.169 |
| 0.15 | 0.175 | 0.416 | 8.9 | 0.117 | **0.265** | −1.213 |
| 0.25 | 0.200 | 0.545 | 12.5 | 0.118 | **0.266** | −1.189 |
| 0.50 | 0.200 | 0.627 | 13.7 | 0.120 | **0.271** | −1.058 |
| 0.75 | 0.225 | 0.616 | 13.0 | 0.121 | **0.275** | −0.976 |
| 1.00（原样） | 0.225 | 0.616 | 13.3 | 0.122 | **0.278** | −0.928 |
| 参考：`raw` | 0.650 | 0.916 | 27.4 | 0.332 | 0.108 | −1.918 |

三条读数：

1. **行为在 α ∈ [0.02, 1.00] 上摆了 3.6 倍（0.625 → 0.175），density 比值只动了 16%
   （0.239 → 0.278），mass 只动了 15%，qk gap 在 −0.93 ~ −1.21 之间晃。**
   50 倍的缩放区间里，注意力统计**基本是常数**，行为却走完了全程。
   这是本轮最强的单条证据：**失败不经过注意力通道**。

2. **压制在 α ≈ 0.10 就饱和了，文档能力要到 α ≈ 0.5 才饱和。** 两条曲线的拐点不同：
   α = 0.10 时压制已达满值（0.175，甚至略低于 α = 1 的 0.225），而 ROUGE-L 只有
   0.334，是满值的 54%。**不存在一个 α 能买回指令遵循同时保住文档**——
   α = 0.02 拿 +0.175 的遵循率换 −0.361 的 ROUGE-L，越往上越亏。

3. **α = 0.10 对应的残差范数约 10**，仍是文本 token（0.14）的 ~70×。也就是说，压制并不
   需要 PISCO 那个 104 的极端范数，只要方向对、幅度到一个很低的阈值就够——
   再次印证 §5 的结论：**承载压制的是方向，不是尺度。**
...
- **不要做** query/instruction-conditioned 的 memory attention 校准、gate、
  logit bias、K 范数匹配。§3、§5 已证明这些动作作用的量与失败无关，§5.1 更直接：
  α 在 50 倍区间上把行为摆了 3.6 倍，而 density / mass / qk gap 基本是常数——
  **注意力统计根本不是这条因果路径上的量。**
- **不要指望用缩放换平衡。** §5.1 显示压制在 α ≈ 0.10 饱和、文档能力到 α ≈ 0.5 才饱和，
  两条曲线拐点不同且方向不利，不存在一个 α 能买回指令遵循同时保住文档。
